    ...
    <div>
      <div className='relative w-full h-full group'>
        <input id='searchBox'
          ...
          className="outline-none peer focus:border-tblue focus:border bg-neutral-900/90 w-full h-full rounded-xl py-4 pl-14 pr-4" />
        <label htmlFor='searchBox'
          className='absolute top-0 left-0 h-full flex items-center justify-center p-4 text-gray-500 peer-focus:text-tblue'> <BsSearch className='w-5 h-5' /></label>
      </div>
    </div>
      ...
      <div>
        {Array.from({ length: 3 }).map((_, i) => (
          ...
          >
            ...
            <button className="rounded-full px-6 py-2 bg-white text-neutral-950 hover:bg-opacity-70 transition duration-200">
              Follow
            </button>
          </div>
        ))}
